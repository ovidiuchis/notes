...
async function init() {
  try {
    ...
    // Update Site Meta
    if(state.data.site) {
      if(state.data.site.title) $('.site-title').innerHTML = state.data.site.title + ` <span class="site-subtitle">${state.data.site.subtitle||''}</span>`;
      if(state.data.site.footerNote) $('#footer-note').textContent = state.data.site.footerNote;
    }
    ...
    console.error(e);
    $('#app').innerHTML = '<p style="color:#999; text-align:center">Eroare la încărcare.</p>';
  }
}
...
function renderHome(container) {
  ...
  const items = state.data.items.filter(i => {
    if (!q) return true;
    return i.title.toLowerCase().includes(q) || (i.note && i.note.toLowerCase().includes(q));
  });
  ...
  if (pinned.length > 0 && !q) { // Only show pinned section specially if not searching? Or always? Let's just follow normal groups for simplicity unless specified. User asked for "Optional Pinned group".
  ...
  sections.forEach(sec => {
    const groupItems = items.filter(i => i.sectionId === sec.id && (!i.pinned || q)); // If pinned, don't show in regular group to avoid dupe, unless searching
...
async function renderPage(container, id) {
  ...
  try {
    const resp = await fetch(item ? item.pagePath : `data/pages/${id}.json`);
